...
struct Credentials {
    ...
    username: String,
    /// The user's password, which must be ascii, at least 8 characters long.
...
fn main() {
    ...
    assert_eq!(
        report
            .get_children_at_path(&path!(create_user.profile.name.middle))
            .count(),
        2
    );
    assert!(report
    ...
    assert_eq!(
        report
            .get_children_at_path(&path!(create_user.profile.hobbies[1]))
            .count(),
        1
    );
    assert!(report
    ...
    assert_eq!(
        report
            .get_children_at_path(&path!(create_user.profile.languages["English"]))
            .count(),
        1
    );
    assert!(report
    ...
    assert_eq!(
        report
            .get_children_at_path(&path!(create_user.credentials.username))
            .count(),
        1
    );
    assert!(report
